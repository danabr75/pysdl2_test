...
class Background(object):
  ...
  def refresh_visible_map_tiles_matrix(self):
      ...
      if map_tile_x_diff > 0:
        ...
        for y_diff in range(0, map_tile_x_diff):
            y_offset = self.map_tile_y - self.visible_map_height_half - y_diff - 1
            self.visible_map_tiles_matrix.insert(0, [None for i in range(self.visible_map_width)])
            # self.map_tile_width: 120
            for w in range(0, self.visible_map_width):
                # x_with_offset = self.map_tile_x + self.visible_map_width_half - w
                x_with_offset = self.map_tile_x + (w - self.visible_map_width_half)
                print("W: " + str(w))
                # W:12
                print(len(self.visible_map_tiles_matrix[0]))
                self.visible_map_tiles_matrix[0][w]
                print("map_tile_y" + str(self.map_tile_y))
                self.map_data[self.map_tile_y]
                print("x_with_offset" + str(x_with_offset))
                self.map_data[self.map_tile_y][x_with_offset]
                print("INSERTING VISIBLE TIPE: " + str(0) + ',' + str(w) + ' with map: ' + str(y_offset) + ',' + str(x_with_offset))
                # INSERTING VISIBLE TIPE: 0,4 with map: 100,93
                self.visible_map_tiles_matrix[0][w] = self.map_data[y_offset][x_with_offset]['map_tile']
                self.visible_map_tiles_matrix[0][w].debug(0, w, True)

        # self.map_data[h][w]['map_tile']
